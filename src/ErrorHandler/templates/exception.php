...
<head>
    <meta charset="utf-8"/>
    <title><?= $this->htmlEncode(get_class($exception)) ?></title>
    <style type="text/css">
/* reset */
...
<body>
    ...
    <div class="header">
        <div class="tools">
            <textarea id="clipboard"><?= $this->htmlEncode($exception) ?></textarea>
            ...
            <h1><span><?= $this->htmlEncode(get_class($exception)) ?></span></h1>

        <h2><?= nl2br($this->htmlEncode($exception->getMessage())) ?></h2>

        <?= $this->renderPreviousExceptions($exception) ?>
    </div>

    <div class="call-stack">
        <?= $this->renderCallStack($exception) ?>
    </div>
